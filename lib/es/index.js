...
import { betterResult, buildByKey, comparator, getOption, isKnownCtor, mergeOptions, mix, paramsToObject, triggerMethod } from 'bbmn-utils';
import { MnObject, Router, history, isClass } from 'bbmn-core';
import $ from 'jquery';
...
var Router$1 = Router.extend({
	...
	getOption: function getOption$$1() {
		return getOption.call.apply(getOption, [this].concat(Array.prototype.slice.call(arguments)));
	},
...
var index$5 = function index$5(Base) {
	return Base.extend({
...
var BasePage = mix(MnObject).with(index$5, StartableMixin, RoutesMixin);
		...
		_.each(events, function (event) {
			_this.on(event, function () {
				_.each(triggersOn, function (parent) {
					parent.triggerMethod('page:' + event, _this);
				});
			});
		});
...
var PagedApp = App.extend({
	...
	_onPageStart: function _onPageStart(page) {
		this.showPage(page);
	},

	showPage: _.noop,
	_onPageStop: function _onPageStop(page) {
		this.showPage(page);
	},
...
var index$1$1 = {
	...
	history: history, historyApi: historyApi
};

export { Router$1 as Router, routeErrorHandler, PagedApp, PageRouter, Page, history, historyApi };
